...
namespace JardinConecta.Infrastructure.Repository
{
    public class ServiceContext : DbContext
    {
        ...
        protected override void OnModelCreating(ModelBuilder builder)
        {
            ...
            builder.Entity<Infante>().HasMany(i => i.Salas).WithOne(s => s.Infante).HasForeignKey(s => s.IdInfante);

            builder.Entity<InfanteSala>().ToTable("Infantes_Salas");
            builder.Entity<InfanteSala>().HasKey(x => new { x.IdInfante, x.IdSala });
            builder.Entity<InfanteSala>().HasOne(x => x.Sala).WithMany(s => s.Infantes).HasForeignKey(x => x.IdSala);
            ...
            builder.Entity<CodigoInvitacion>().HasMany(c => c.Infantes).WithOne(x => x.CodigoInvitacion).HasForeignKey(x => x.IdCodigoInvitacion);

            builder.Entity<CodigoInvitacionInfante>().ToTable("CodigoInvitacion_Infantes");
            builder.Entity<CodigoInvitacionInfante>().HasKey(x => new { x.IdCodigoInvitacion, x.IdInfante });
            builder.Entity<CodigoInvitacionInfante>().HasOne(x => x.Infante).WithMany().HasForeignKey(x => x.IdInfante);
